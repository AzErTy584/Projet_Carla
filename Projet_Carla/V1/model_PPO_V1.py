...
class PPOActorCritic(nn.Module):
    def __init__(
        ...
        self.state_dim = lidar_dim + 7  # collision(1) + speed(1) + lane_offset(1) + lane_angle(1) + goal_direction(2) + goal_distance(1)
    ...
    def forward(self, state_tensor):
        """
        state_tensor : torch.Tensor shape (B, state_dim)
        """
...
# ----------------------------
def save_model(model, optimizer, episode, path="V1\model_checkpoint_PPO.pth"):
    try:
        torch.save({
            'episode': episode,
