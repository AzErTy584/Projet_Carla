...
from model_PPO_V6 import PPOActorCritic, RolloutBuffer, load_model, save_model, ppo_update

# Commande pour lancé l'entrainnement : CarlaUE4.exe -RenderOffScreen -carla-port=2000 -nosound
# Commande pour lancé la visualisation : CarlaUE4.exe -fps=20 -world-port=2000
...
def encode_state(state_dict, device="cpu"):
    ...
    state = np.concatenate([
        ...
        state_dict["goal_direction"], # (2,) Plus utilisé par le modèle, issue des précédentes versions
        state_dict["goal_distance"],  # (1,) Plus utilisé par le modèle, issue des précédentes versions
    ]).astype(np.float32)

    return torch.tensor(state, device=device).unsqueeze(0)

# Fonction de nettoyage GPU afin d'éviter les crahs liés à la mémoire
...
if __name__ == "__main__":
    ...
    # Charger ou créer modèle si indisponible
    start_episode = 0
    model_charged = False
    model = None
    optimizer = None
    
    if os.path.exists("V0/model_checkpoint_PPO.pth"):
        try:
            model, optimizer, start_episode = load_model("V0/model_checkpoint_PPO.pth", device=device)
            model_charged = True
    ...
    for episode in range(start_episode, num_episodes):
        ...
        # Sauvegarder les récompenses de l'épisode, afin de pouvoir les tracer plus tard dans un graphique
        df_rewards.append({
            "episode": episode,
            "reward": total_reward,
        ...
        # Sauvegarder le modèle tous les 3 épisodes, afin d'avoir des points de sauvegarde réguliers sans surcharger le disque
        if episode % 3 == 0:
